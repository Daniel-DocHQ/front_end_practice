...
export default class OrderPage {
	...
	fill_account_details(source = 'dochq', email = user.users[user_index].email) {
		let first_name = user.users[user_index].first_name;
		let last_name = user.users[user_index].last_name;
		let date_of_birth = user.users[user_index].date_of_birth;
	...
	// Random user data is generated by default
	fill_shipping_address(source = 'dochq', country_code = user.users[user_index].country_code, phone = user.users[user_index].phone) {

		let postcode = user.users[user_index].address.postcode;
		let address_1 = user.users[user_index].address.street;
		let city = user.users[user_index].address.city;
		let country = user.users[user_index].address.country;
	...
	fill_payment_data() {
		...
		const braintree_card_num = "4000'0000'0000'1000", card_expiry = "12/24", card_cvv = "123";
		const stripe_card_num = "4111'1111'1111'1111";

		cy.get("#payment-iframe").then(($payment) => {
		
			if($payment.find("#braintree-iframe").length > 0) {
				// ------------------------------------  Braintree iframe ------------------------------------------------
				cy.getIframe('#braintree-hosted-field-number').find('input[name="credit-card-number"]').fill(braintree_card_num)
				...
				cy.get('.__PrivateStripeElement > iframe')
					.iframeLoaded()
					.its('document')
					.getInDocument('input[name="number"]').type(stripe_card_num)
